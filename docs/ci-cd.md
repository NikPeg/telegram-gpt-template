...
| `REMINDER_TIME` | `19:15` | Время напоминания в формате HH:MM (МСК) |
| `REMINDER_WEEKDAYS` | `` (пусто) | Дни недели через запятую (0=Пн, 6=Вс), пусто = все дни. Например: `0,1,2,3,4` для будних дней |
| `REMINDER_CHECK_INTERVAL` | `900` | Интервал проверки напоминаний в секундах (по умолчанию 15 минут = 900 секунд) |
| `FEEDBACK_FORM_URL` | `` (пусто) | Ссылка на Google форму для обратной связи. Если указана, бот будет предлагать её в напоминаниях типа feedback |
...
📖 **[Best Practices и рекомендации →](best-practices.md)**
