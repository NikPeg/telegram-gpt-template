...
```env
# Напоминания
DELAYED_REMINDERS_HOURS=2      # Напоминание через N часов после последнего сообщения
DELAYED_REMINDERS_MINUTES=0    # + дополнительные минуты
FROM_TIME=9                     # Начало временного окна (час, 0-23)
TO_TIME=23                      # Конец временного окна (час, 0-23)
TIMEZONE_OFFSET=3               # Часовой пояс (UTC+3 для МСК)
```

**Пример:** С настройками `DELAYED_REMINDERS_HOURS=2` и временным окном `9-23`:
- Если пользователь написал в 14:00, напоминание придет в 16:00
- Если пользователь написал в 22:00, напоминание отложится до 9:00 следующего дня
