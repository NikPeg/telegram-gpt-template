...
- [Проблемы с напоминаниями](#проблемы-с-напоминаниями)
...
cat .env | grep -E "DELAYED_REMINDERS|FROM_TIME|TO_TIME"
```
...
FROM_TIME=9    # Начало периода напоминаний (час)
TO_TIME=23     # Конец периода напоминаний (час)

# Проверьте часовой пояс
TIMEZONE_OFFSET=3  # UTC+3 для МСК
...
**Причина:** Неправильный `TIMEZONE_OFFSET`
