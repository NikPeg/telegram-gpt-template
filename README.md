...
Бот поддерживает гибкую настройку уровней логирования через переменные окружения.

### Настройка уровней логирования в `.env`:

```env
# Основной уровень (DEBUG, INFO, WARNING, ERROR, CRITICAL)
LOG_LEVEL=INFO

# Уровень для консоли (по умолчанию: берется из LOG_LEVEL)
CONSOLE_LOG_LEVEL=INFO

# Уровень для файла debug.log (по умолчанию: DEBUG)
FILE_LOG_LEVEL=DEBUG
```

### Типы логов промптов:

- **PROMPT_FULL_*** (DEBUG) — полные промпты со всей историей диалога
- **PROMPT_SYSTEM_*** (INFO) — только системные промпты без истории
- **PROMPT_STATS_*** (INFO) — статистика промптов (количество сообщений, символов)
...
**Отладка промптов:**
```env
FILE_LOG_LEVEL=DEBUG  # для просмотра полных промптов
```
...
📖 **[Подробная документация по логированию →](docs/logging.md)**
