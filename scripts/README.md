...
YC_INSTANCE_IP=89.169.165.5 YC_INSTANCE_USER=ubuntu ./scripts/prepare_github_secrets.sh
```

**Что делает:**

1. ✅ Проверяет наличие Yandex Cloud CLI (`yc`)
2. 📦 Получает список Container Registry и предлагает выбрать
3. 👤 Получает список Service Accounts или создаёт новый
4. 🔑 Автоматически создаёт ключ для Service Account
5. 🖥️ Запрашивает IP сервера и SSH пользователя (или берёт из env)
6. 🔍 Автоматически находит SSH ключ
