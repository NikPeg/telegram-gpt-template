...
from services.llm_service import process_user_image, process_user_message
...
async def handle_video_message(message: types.Message):
    """Обработка видео и кружочков."""
    # Игнорируем сообщения из ADMIN_CHAT
    if message.chat.id == ADMIN_CHAT:
        return

    logger.info(
        f"USER{message.chat.id} отправил видео (тип: {message.content_type}), не обрабатываем"
    )
    await message.answer(
        "Извини, я пока не умею обрабатывать видео. Отправь мне текстовое сообщение или фотографию."
    )
